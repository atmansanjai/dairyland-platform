...
@Slf4j
public class CustomerInvoiceScheduler {

    private final JobOperator jobOperator;
    private final Job generateCustomerInvoiceJob;

    @Scheduled(cron = "0 0 0 * * *")
    public void runCustomerInvoiceJob() {
        try {
            ...
            JobParameters jobParameters = new JobParametersBuilder().addString("timestamp", String.valueOf(System.currentTimeMillis()))
                                                                    .toJobParameters();
            jobOperator.start(generateCustomerInvoiceJob, jobParameters);
